...
world_map = folium.Map(location=[-11.1855,-40.5361],zoom_start=6.5)
cidades = folium.FeatureGroup()
for cidade, coord in dic.items():
    cidades.add_child(
        folium.Marker(coord, popup=cidade)
    )
world_map.add_child(cidades)
world_map = world_map._repr_html_()
world_map

bahia = ' Camaçari, Pindobaçu, Utinga, Seabra, Serrolândia, Iuiu, Salvador, Ibitiara, Saúde, Iaçu, Várzea Nova, Candeal, Capim Grosso, Ibotirama, A. Dourada, Sobradinho, Jacobina'.split(",")
bahia = sorted(bahia)
sergipe = ' Carira, Aracaju, São Cristóvão, N. Sª Da Glória, Neópolis, Simão Dias, Frei Paulo, Campo do Brito, Macambira'.split(",")
